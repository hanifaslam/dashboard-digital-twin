...
export function useLecturerListQuery(id: string, q?: string) {
  ...
  useEffect(() => {
    ...
    const handleData = (payload: { room_id: string; lecturers: LecturerResponse[] }) => {
      if (payload.room_id === id) {
        queryClient.setQueryData(LECTURER_QUERY_KEY.list(id, q), payload.lecturers);
      }
    };
  ...
  return useQuery({
    ...
    queryFn: () => {
      return new Promise<LecturerResponse[]>((resolve) => {
        const onData = (payload: { room_id: string; lecturers: LecturerResponse[] }) => {
          if (payload.room_id === id) {
